...
mod mariadb_tests {
    ...
    use postings_api::service::account_stmt_service::AccountStmtService;
    use postings_db::models::posting_line::PostingLine as PostingLineModel;
    use postings_db_mariadb::repositories::{
        account_stmt_repository::MariaDbAccountStmtRepository,
        chart_of_account_repository::MariaDbChartOfAccountRepository,
        ledger_account_repository::MariaDbLedgerAccountRepository,
        ledger_repository::MariaDbLedgerRepository,
        posting_line_repository::MariaDbPostingLineRepository,
        posting_repository::MariaDbPostingRepository,
        posting_trace_repository::MariaDbPostingTraceRepository,
    };
    use postings_logic::services::{
        account_stmt_service::AccountStmtServiceImpl, shared_service::SharedService,
    };

    async fn setup_test_data(pool: &MySqlPool) -> anyhow::Result<(LedgerAccount, Ledger)> {
        let coa = ChartOfAccount {
            named: Named {
                id: Uuid::new_v4(),
                name: "Test COA".to_string(),
        ...
        sqlx::query("INSERT INTO ledger_account (id, name, ledger_id, coa_id, balance_side, category, created, user_details) VALUES (?, ?, ?, ?, ?, ?, ?, ?)")
    ...
    async fn test_read_stmt(pool: MySqlPool) -> anyhow::Result<()> {
        let _ = env_logger::builder().is_test(true).try_init();
        ...
        sqlx::query("INSERT INTO posting_line (id, account_id, debit_amount, credit_amount, details, record_time, opr_id, pst_time, pst_type, pst_status, hash) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)")
            .bind(line1.id.to_string())
            .bind(line1.account_id.to_string())
            .bind(&line1.debit_amount.to_string())
            .bind(&line1.credit_amount.to_string())
            .bind(&line1.details)
            .bind(line1.record_time)
            .bind(&line1.opr_id)
            .bind(line1.pst_time)
            ...
            .await?;
        sqlx::query("INSERT INTO posting_line (id, account_id, debit_amount, credit_amount, details, record_time, opr_id, pst_time, pst_type, pst_status, hash) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)")
            .bind(line2.id.to_string())
            .bind(line2.account_id.to_string())
            .bind(&line2.debit_amount.to_string())
            .bind(&line2.credit_amount.to_string())
            .bind(&line2.details)
            .bind(line2.record_time)
            .bind(&line2.opr_id)
            .bind(line2.pst_time)
    ...
    #[sqlx::test(migrations = "../postings-db-mariadb/migrations")]
    async fn test_create_stmt_no_closed_statement(pool: MySqlPool) -> anyhow::Result<()> {
        let _ = env_logger::builder().is_test(true).try_init();
        dotenvy::from_filename(".env.mariadb").ok();
        // Arrange
        let (ledger_account, _ledger) = setup_test_data(&pool).await?;
        ...
        // Assert
        let stmt_model = sqlx::query_as::<_, postings_db::models::account_stmt::AccountStmt>("SELECT * FROM account_stmt WHERE id = ?")
    ...
    #[sqlx::test(migrations = "../postings-db-mariadb/migrations")]
    async fn test_close_stmt(pool: MySqlPool) -> anyhow::Result<()> {
        let _ = env_logger::builder().is_test(true).try_init();
        dotenvy::from_filename(".env.mariadb").ok();
        // Arrange
        let (ledger_account, _ledger) = setup_test_data(&pool).await?;
    ...
    fn create_service(pool: MySqlPool) -> AccountStmtServiceImpl {

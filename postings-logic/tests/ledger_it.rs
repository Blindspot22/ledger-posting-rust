...
mod mariadb_tests {
    ...
    async fn setup_coa(pool: &MySqlPool) -> anyhow::Result<ChartOfAccount> {
        let coa = ChartOfAccount {
            named: Named {
                id: Uuid::new_v4(),
                name: "Test COA".to_string(),
    ...
    async fn setup_ledger(pool: &MySqlPool, coa: &ChartOfAccount) -> anyhow::Result<Ledger> {
        let ledger = Ledger {
            named: Named {
                id: Uuid::new_v4(),
                name: "Test Ledger".to_string(),
    ...
    fn create_service(pool: MySqlPool) -> LedgerServiceImpl {
    ...
    async fn test_new_ledger(pool: MySqlPool) -> anyhow::Result<()> {
    ...
    #[sqlx::test(migrations = "../postings-db-mariadb/migrations")]
    async fn test_new_ledger_account_with_parent_inheritance(pool: MySqlPool) -> anyhow::Result<()> {
        dotenvy::from_filename(".env.mariadb").ok();
        // Arrange
        let coa = setup_coa(&pool).await?;
        let ledger = setup_ledger(&pool, &coa).await?;
    ...
    #[sqlx::test(migrations = "../postings-db-mariadb/migrations")]
    async fn test_new_ledger_account_no_parent_no_category_fails(pool: MySqlPool) -> anyhow::Result<()> {
        dotenvy::from_filename(".env.mariadb").ok();
        // Arrange
        let coa = setup_coa(&pool).await?;
        let ledger = setup_ledger(&pool, &coa).await?;
    ...
    #[sqlx::test(migrations = "../postings-db-mariadb/migrations")]
    async fn test_new_ledger_account_no_parent_uses_category_default_balance_side(pool: MySqlPool) -> anyhow::Result<()> {
        dotenvy::from_filename(".env.mariadb").ok();
        // Arrange
        let coa = setup_coa(&pool).await?;
        let ledger = setup_ledger(&pool, &coa).await?;
    ...
    #[sqlx::test(migrations = "../postings-db-mariadb/migrations")]
    async fn test_find_ledger_account_by_id_with_parent_hierarchy(pool: MySqlPool) -> anyhow::Result<()> {
        dotenvy::from_filename(".env.mariadb").ok();
        // Arrange
        let coa = setup_coa(&pool).await?;
        let ledger = setup_ledger(&pool, &coa).await?;
